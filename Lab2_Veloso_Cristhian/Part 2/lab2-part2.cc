/*
 * Este é o script para o Lab 2, Parte 2.
 * Baseado em lab2-part1.cc e modificado para a topologia
 * de 2 destinos e para testar a Justiça de RTT.
 */
...
#include "ns3/flow-monitor-module.h" // Incluído para Ipv4FlowClassifier
...
// Funções de Rastreamento (CWND, etc.) podem ser omitidas se não forem usadas,
// mas as deixaremos caso sejam necessárias para depuração.
// O roteiro da Parte 2 foca apenas no goodput.
...
int main (int argc, char *argv[])
{
  ...
  uint16_t num_flows = 2; // O padrão agora é 2
  uint32_t run = 0; // Novo parâmetro para o RNG
  
  bool flow_monitor = false; // Desligado por padrão
  ...
  // --- 2. Configuração do TCP ---
  // (Remoções de SACK, RcvBuf, SndBuf, RecoveryType já foram feitas no script anterior)
  TypeId tcpTid;
  NS_ABORT_MSG_UNLESS (TypeId::LookupByNameFailSafe (transport_prot, &tcpTid), "TypeId " << transport_prot << " not found");
  Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TypeId::LookupByName (transport_prot)));
  
  // --- 3. Configuração dos Tempos de Simulação ---
  double start_time = 1.0;  // Início dos fluxos
  double stop_time = 20.0; // Fim da simulação

  // --- 4. Criação da Topologia (5 Nós, 4 Links) ---
  // [source] -- (link 0) -- [r1] --(gargalo, link 1)-- [r2] -- (link 2) -- [dest1]
  //                                                      |
  //                                                      -- (link 3) -- [dest2]
  ...
  // --- 5. Configuração do Modelo de Erro ---
  Ptr<RateErrorModel> em = CreateObject<RateErrorModel> ();
  em->SetAttribute ("ErrorRate", DoubleValue (errorRate));

  // --- 6. Configuração dos Links Ponto-a-Ponto ---
  ...
  // --- 7. Instalação da Pilha de Internet e Roteamento ---
  InternetStackHelper stack;
  stack.Install (nodes); // Instala em todos os 5 nós
  ...
  // --- 8. Configuração das Aplicações (Sink e Source) ---
  ...
  // --- 11. Processamento de Saída (Goodput Médio por Destino) ---
  if (flow_monitor)
    {
      ...
      // Imprime as linhas de média que o script de automação irá capturar
